import numpy as np
import cv2 as cv

# Relative paths work from the main folder OPENCV WITH PYTHON 
haar_cascade = cv.CascadeClassifier("haar_face.xml")

# List of names
people = ['Ben Afflek', 'Elton John', 'Jerry Seinfield', 'Madonna', 'Mindy Kaling']

# # Load files created during training
# features = np.load(r"Face Recognition\features.npy", allow_pickle=True)
# labels = np.load(r"Face Recognition\labels.npy")

face_recognizer = cv.face.LBPHFaceRecognizer_create()
face_recognizer.read(r"Face Recognition\face_trained.yml")

# Read an image
img = cv.imread(r"Face Recognition\Faces\val\elton_john\4.jpg")

gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
cv.imshow("Person", gray)

# 1. Detect the face in the image
faces_rect = haar_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=4)

for (x, y, w, h) in faces_rect:
    faces_roi = gray[y:y+h, x:x+h]

    label, confidence = face_recognizer.predict(faces_roi)
    print(f"Label is {people[label]} with a confidence of {confidence}")

    cv.putText(img, str(people[label]), (20, 20), cv.FONT_HERSHEY_COMPLEX, 1.0, (0, 255, 0), thickness=2)
    cv.rectangle(img, (x,y), (x+w, y+h), (0, 255, 0), thickness=2)

cv.imshow("Detected Face", img)
cv.waitKey(0)